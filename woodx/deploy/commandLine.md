...
## 生成通道的创世交易
``` bash
configtxgen -profile TwoOrgChannel -outputCreateChannelTx ./config/woodchannel.tx -channelID woodchannel
```

## 生成组织关于通道的锚节点（主节点）交易(可忽略)
``` bash
...
## 创建通道
``` bash
peer channel create -o orderer.wood.com:7050 -c woodchannel -f /etc/hyperledger/config/woodchannel.tx
...
## 加入通道
``` bash
peer channel join -b woodchannel.block
...
## 链码安装
``` bash
peer chaincode install -n woods -v 1.0.0 -l golang -p github.com/chaincode/woodsExchange
```

## 链码实例化
``` bash
peer chaincode instantiate -o orderer.wood.com:7050 -C woodchannel -n woods -l golang -v 1.0.0 -c '{"Args":["init"]}'
```

## 链码交互
``` bash
peer chaincode invoke -C woodchannel -n woods -c '{"Args":["userRegister", "user1", "user1"]}'
peer chaincode invoke -C woodchannel -n woods -c '{"Args":["queryUser", "user1"]}'
